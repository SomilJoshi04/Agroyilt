...
const LiveBookingCard = ({ hasBottomNav }) => {
  ...
  if (
    !activeBooking ||
    isDismissed ||
    location.pathname.includes('/farmer-worker-request') ||
    ...
    <AnimatePresence>
      <motion.div
        ...
        onClick={() => {
          ...
          const extractId = (val) => (val && typeof val === 'object' ? val._id || val.id : val);
          const targetParentId = extractId(activeBooking.workerRequestId) || extractId(activeBooking.parentRequestId);
          const isWorker = activeBooking.providerType === 'WORKER' || (activeBooking.bookingNumber && activeBooking.bookingNumber.startsWith('WRK-'));

          if (targetParentId) {
            // Multi-Worker booking -> Always navigate to the unified Multi-Worker Tracking Hub
            navigate(`/user/booking/${targetParentId}/track`);
            return;
          }

          if (isWorker) {
            // Any worker booking -> Navigate to tracking hub which auto-resolves parent multi-workers
