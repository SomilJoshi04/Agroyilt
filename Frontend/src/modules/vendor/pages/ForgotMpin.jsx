...
const ForgotMpin = () => {
  ...
  const handleSendOtp = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await vendorAuthService.sendOTP(phoneNumber, null, 'forgotMpin');
      if (response.success) {
      ...
      if (error.response?.status === 404) {
        navigate('/vendor/signup', { state: { phone: phoneNumber } });
      } else {
        navigate('/vendor/login');
      }
    } finally {
  ...
  const verifyOtp = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await vendorAuthService.verifyLogin({
        phone: phoneNumber,
        otp: otpValue
      });
      if (response.success) {
  ...
  const handleReset = async (e) => {
    ...
    setIsLoading(true);
    try {
      const response = await vendorAuthService.resetMpin({
